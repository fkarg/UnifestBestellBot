from telegram import InlineKeyboardMarkup, ReplyKeyboardMarkup, ReplyKeyboardRemove, Update, Bot
...
REQUEST, MONEY, CUPS, BEER, COCKTAIL, COLLECT, BILLS, AMOUNT, COIN2, COIN1, COIN0, SHOT, NORMAL, RETRIEVAL, FREE = range(15)
...
def request(update: object, context: CallbackContext) -> int:
    update.message.reply_text(
            ...
            reply_markup=ReplyKeyboardMarkup(
                REQUEST_OPTIONS, one_time_keyboard=True, input_field_placebo="What to create a ticket about?"
            ),
        )
...
def money(update: object, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
            ...
            reply_markup=ReplyKeyboardMarkup(
                MONEY_OPTIONS, one_time_keyboard=True, input_field_placebo="What is it about?"
            ),
        )
...
def collect(update: object, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Sending someone to collect Money.",
...
def free(update: object, context: CallbackContext) -> int:
    ...
    log.info(update.message.text)
    update.message.reply_text("Thank you, your ticket has been created.",
            reply_markup=ReplyKeyboardRemove(),
    )
    return ConversationHandler.END
